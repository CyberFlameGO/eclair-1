...
class Bolt12InvoiceSpec extends AnyFunSuite {
  ...
  def createDirectPath(sessionKey: PrivateKey, nodeId: PublicKey, pathId: ByteVector): (BlindedRoute, PaymentInfo) = {
    ...
    (Sphinx.RouteBlinding.create(sessionKey, Seq(nodeId), Seq(selfPayload)).route, PaymentInfo(0 msat, 0, CltvExpiryDelta(0), 0 msat, 10_000_000_000L msat, Features.empty))
  }
  ...
  test("check invoice signature") {
    ...
    val invoice = Bolt12Invoice(request, randomBytes32(), nodeKey, Features.empty, Seq(createDirectPath(randomKey(), nodeKey.publicKey, randomBytes32())))
  ...
  test("check that invoice matches offer") {
    ...
    val withOtherNodeId = signInvoice(Bolt12Invoice(TlvStream(invoice.records.records.map { case OfferNodeId(_) => OfferNodeId(otherNodeKey.publicKey) case x => x }.toSeq)), otherNodeKey)
  ...
  test("check that invoice matches invoice request") {
    ...
    val invoice = Bolt12Invoice(request, randomBytes32(), nodeKey, Features(VariableLengthOnion -> Mandatory, BasicMultiPartPayment -> Optional), Seq(createDirectPath(randomKey(), nodeKey.publicKey, randomBytes32())))
    assert(invoice.isValidFor(request))
    val withInvalidFeatures = signInvoice(Bolt12Invoice(TlvStream(invoice.records.records.map { case InvoiceRequestFeatures(_) => InvoiceRequestFeatures(Features(VariableLengthOnion -> Mandatory, BasicMultiPartPayment -> Mandatory)) case x => x }.toSeq)), nodeKey)
    ...
    val withPayerDetails = Bolt12Invoice(requestWithPayerDetails, randomBytes32(), nodeKey, Features.empty, Seq(createDirectPath(randomKey(), nodeKey.publicKey, randomBytes32())))
  ...
  test("decode invalid invoice") {
    ...
    val tlvs = Seq[InvoiceTlv](
      ...
      InvoicePaths(Seq(createDirectPath(randomKey(), randomKey().publicKey, randomBytes32())._1)),
  ...
  test("encode/decode invoice with many fields") {
    ...
    val tlvs = TlvStream[InvoiceTlv](Seq(
      ...
      InvoicePaths(Seq(path._1)),
      InvoiceBlindedPay(Seq(payInfo)),
    ...
    assert(invoice.toString == "")
    ...
    assert(codedDecoded.blindedPaths == Seq(path._1))
    ...
    assert(codedDecoded.invoiceRequest.metadata.contains(payerInfo))
  ...
  test("minimal tip") {
    ...
    val invoice = Bolt12Invoice(decodedRequest, preimage, nodeKey, Features.empty, Seq(createDirectPath(randomKey(), nodeKey.publicKey, hex"")))
    ...
    val encodedInvoice = ""
  ...
  test("minimal offer") {
    ...
    assert(decodedRequest.unsigned.records.filterNot(_.isInstanceOf[InvoiceRequestMetadata]) == request.unsigned.records.filterNot(_.isInstanceOf[InvoiceRequestMetadata]))
    assert(request.isValidFor(offer))
    val invoice = Bolt12Invoice(decodedRequest, preimage, nodeKey, Features.empty, Seq(createDirectPath(randomKey(), nodeKey.publicKey, hex"747e01a7152169b058a1fbc0024c254077db7e399308483e0c30e2352ba1d6cc")))
    assert(Bolt12Invoice.fromString(invoice.toString).get.records == invoice.records)
    assert(invoice.isValidFor(decodedRequest))
    // Invoice generation is not reproducible as the timestamp and blinding point will change but all other fields should be the same.
    val encodedInvoice = ""
    val decodedInvoice = Bolt12Invoice.fromString(encodedInvoice).get
    assert(decodedInvoice.amount == invoice.amount)
    assert(decodedInvoice.nodeId == invoice.nodeId)
    assert(decodedInvoice.paymentHash == invoice.paymentHash)
  ...
  test("offer with quantity") {
    ...
    assert(decodedRequest.unsigned.records.filterNot(_.isInstanceOf[InvoiceRequestMetadata]) == request.unsigned.records.filterNot(_.isInstanceOf[InvoiceRequestMetadata]))
    assert(request.isValidFor(offer))
    val invoice = Bolt12Invoice(decodedRequest, preimage, nodeKey, Features.empty, Seq(createDirectPath(randomKey(), nodeKey.publicKey, hex"9134d86e269a13203bd85bb3fd05bf396b72fcb9fd5206e3a392f6a0ab94011d")))
    assert(Bolt12Invoice.fromString(invoice.toString).get.records == invoice.records)
    assert(invoice.isValidFor(decodedRequest))
    // Invoice generation is not reproducible as the timestamp and blinding point will change but all other fields should be the same.
    val encodedInvoice = ""
    val decodedInvoice = Bolt12Invoice.fromString(encodedInvoice).get
    assert(decodedInvoice.amount == invoice.amount)
    assert(decodedInvoice.nodeId == invoice.nodeId)
    assert(decodedInvoice.paymentHash == invoice.paymentHash)
