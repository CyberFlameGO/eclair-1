...
object OutgoingPaymentPacket {
  ...
  def buildOnion(packetPayloadLength: Int, payloads: Seq[NodePayload], associatedData: ByteVector32): Try[Sphinx.PacketAndSecrets] = {
    ...
    Sphinx.create(sessionKey, packetPayloadLength, nodeIds, payloadsBin, Some(associatedData))
  }
