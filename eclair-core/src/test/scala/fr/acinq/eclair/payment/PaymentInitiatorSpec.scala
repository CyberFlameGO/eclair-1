...
class PaymentInitiatorSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("forward payment with user custom tlv records") { f =>
    ...
    assert(payment.recipient.targetNodeId == invoice.nodeId)
    assert(payment.recipient.targetTotalAmount == finalAmount)
    assert(payment.recipient.isInstanceOf[ClearRecipient])
    assert(payment.recipient.asInstanceOf[ClearRecipient].expiry == req.invoice.minFinalCltvExpiryDelta.toCltvExpiry(nodeParams.currentBlockHeight + 1))
  ...
  test("forward keysend payment") { f =>
    ...
    val payment = payFsm.expectMsgType[PaymentLifecycle.SendPayment]
    assert(payment.amount == finalAmount)
    assert(payment.recipient.targetNodeId == c)
    assert(payment.recipient.targetTotalAmount == finalAmount)
    assert(payment.recipient.isInstanceOf[SpontaneousRecipient])
    assert(payment.recipient.asInstanceOf[SpontaneousRecipient].expiry == Channel.MIN_CLTV_EXPIRY_DELTA.toCltvExpiry(nodeParams.currentBlockHeight + 1))
  ...
  test("forward payment with pre-defined route") { f =>
    ...
    val request = SendPaymentToRoute(finalAmount, finalAmount, invoice, route, None, None, None)
    sender.send(initiator, request)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
    payFsm.expectMsg(SendPaymentConfig(payment.paymentId, payment.parentId, None, paymentHash, c, Upstream.Local(payment.paymentId), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = false))
    payFsm.expectMsg(PaymentLifecycle.SendPaymentToRoute(initiator, Left(route), finalAmount, ClearRecipient(invoice, finalAmount, finalExpiryDelta.toCltvExpiry(nodeParams.currentBlockHeight + 1), Nil)))
  ...
  test("forward single-part payment when multi-part deactivated", Tag(Tags.DisableMPP)) { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    payFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, c, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
    payFsm.expectMsg(PaymentLifecycle.SendPaymentToNode(initiator, finalAmount, ClearRecipient(invoice, finalAmount, req.finalExpiry(nodeParams), Nil), 1, nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams))
  ...
  test("forward multi-part payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    multiPartPayFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, c, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
    multiPartPayFsm.expectMsg(SendMultiPartPayment(initiator, ClearRecipient(invoice, finalAmount + 100.msat, req.finalExpiry(nodeParams), Nil), 1, nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams))
  ...
  test("forward multi-part payment with pre-defined route") { f =>
    ...
    val req = SendPaymentToRoute(finalAmount / 2, finalAmount, invoice, route, None, None, None)
    sender.send(initiator, req)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
    payFsm.expectMsg(SendPaymentConfig(payment.paymentId, payment.parentId, None, paymentHash, c, Upstream.Local(payment.paymentId), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = false))
    ...
    assert(msg.recipient.targetNodeId == c)
    assert(msg.recipient.targetTotalAmount == finalAmount)
    assert(msg.recipient.isInstanceOf[ClearRecipient])
    assert(msg.recipient.asInstanceOf[ClearRecipient].expiry == req.finalExpiry(nodeParams))
  ...
  test("forward single-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    payFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, invoice.nodeId, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
    ...
    val recipient = payment.recipient.asInstanceOf[BlindedRecipient]
    assert(recipient.nodeId == invoice.nodeId)
    assert(recipient.totalAmount == finalAmount)
    assert(recipient.targetTotalAmount == finalAmount)
    assert(recipient.extraEdges.nonEmpty)
    assert(recipient.expiry == req.finalExpiry(nodeParams))
  ...
  test("forward multi-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    multiPartPayFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, invoice.nodeId, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
    val payment = multiPartPayFsm.expectMsgType[SendMultiPartPayment]
    assert(payment.recipient.isInstanceOf[BlindedRecipient])
    val recipient = payment.recipient.asInstanceOf[BlindedRecipient]
    assert(recipient.nodeId == invoice.nodeId)
  ...
  test("forward trampoline payment") { f =>
    ...
    assert(msg.recipient.targetTotalAmount == finalAmount + trampolineFees)
    assert(msg.recipient.targetNodeId == b)
    ...
    assert(msg.recipient.asInstanceOf[ClearTrampolineRecipient].expiry.toLong == currentBlockCount + 9 + 1)
  ...
  test("retry trampoline payment") { f =>
    ...
    assert(msg1.recipient.targetTotalAmount == finalAmount + 21_000.msat)

    sender.send(initiator, GetPayment(Left(id)))
    sender.expectMsgType[PaymentIsPending]

    // Simulate a failure which should trigger a retry.
    multiPartPayFsm.send(initiator, PaymentFailed(cfg.parentId, invoice.paymentHash, Seq(RemoteFailure(msg1.recipient.targetTotalAmount, Nil, Sphinx.DecryptedFailurePacket(b, TrampolineFeeInsufficient)))))
    multiPartPayFsm.expectMsgType[SendPaymentConfig]
    val msg2 = multiPartPayFsm.expectMsgType[SendMultiPartPayment]
    assert(msg2.recipient.totalAmount == finalAmount)
    assert(msg2.recipient.targetTotalAmount == finalAmount + 25_000.msat)
  ...
  test("retry trampoline payment and fail") { f =>
    ...
    val failed = PaymentFailed(cfg.parentId, invoice.paymentHash, Seq(RemoteFailure(msg2.recipient.targetTotalAmount, Nil, Sphinx.DecryptedFailurePacket(b, TemporaryNodeFailure))))
  ...
  test("retry trampoline payment and fail (route not found)") { f =>
    ...
    val failed = PaymentFailed(cfg.parentId, invoice.paymentHash, Seq(RemoteFailure(msg1.recipient.targetTotalAmount, Nil, Sphinx.DecryptedFailurePacket(b, TrampolineFeeInsufficient))))
    multiPartPayFsm.send(initiator, failed)
    multiPartPayFsm.expectMsgType[SendPaymentConfig]
    val msg2 = multiPartPayFsm.expectMsgType[SendMultiPartPayment]
    assert(msg2.recipient.targetTotalAmount == finalAmount + 25_000.msat)
  ...
  test("forward trampoline payment with pre-defined route") { f =>
    ...
    val req = SendPaymentToRoute(finalAmount + trampolineAttempt.fees, finalAmount, invoice, route, None, None, Some(trampolineAttempt))
    sender.send(initiator, req)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
