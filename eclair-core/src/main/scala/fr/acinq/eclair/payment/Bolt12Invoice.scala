...
case class Bolt12Invoice(records: TlvStream[InvoiceTlv]) extends Invoice {
  ...
  override val nodeId: Crypto.PublicKey = invoiceRequest.offer.nodeId
  ...
  val blindedPayInfo: Seq[PaymentInfo] = records.get[InvoiceBlindedPay].get.paymentInfo
...
object Bolt12Invoice {
  ...
  def apply(request: InvoiceRequest,
    ...
    val tlvs: Seq[InvoiceTlv] = removeSignature(request.records).records.toSeq ++ Seq(
      Some(InvoicePaths(paths.map(_._1))),
      Some(InvoiceBlindedPay(paths.map(_._2))),
      ...
      if (!features.isEmpty) Some(InvoiceFeatures(features.unscoped())) else None).flatten
  ...
  def validate(records: TlvStream[InvoiceTlv]): Either[InvalidTlvPayload, Bolt12Invoice] = {
    ...
    if (records.get[InvoiceCode].nonEmpty) return Left(ForbiddenTlv(UInt64(176)))
