...
import fr.acinq.eclair.{CltvExpiryDelta, Features, InvoiceFeature, MilliSatoshiLong, randomBytes, randomBytes32, randomKey}
...
class OffersDbSpec extends AnyFunSuite {
  ...
  test("add attempts to pay offers and list them") {
    forAllDbs { dbs =>
      ...
      val dummyRoute = RouteBlinding.create(randomKey(), Seq(randomKey().publicKey), Seq(randomBytes(5))).route
      val invoice2 = Bolt12Invoice(offer1, request2, randomBytes32(), a, CltvExpiryDelta(3), Features.empty, Seq(dummyRoute))
      val invoice3 = Bolt12Invoice(offer2, request3, randomBytes32(), b, CltvExpiryDelta(4), Features.empty, Seq(dummyRoute))
      val invoice4 = Bolt12Invoice(offer3, request4, randomBytes32(), a, CltvExpiryDelta(5), Features.empty, Seq(dummyRoute))
