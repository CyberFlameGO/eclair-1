...
class RelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("relay an htlc-add") { f =>
    ...
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, priv_a.privateKey, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, hops), ClearRecipient(e, Features.empty, finalAmount, finalExpiry, paymentSecret))
  ...
  test("relay an htlc-add at the final node to the payment handler") { f =>
    ...
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, priv_a.privateKey, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, hops.take(1)), ClearRecipient(b, Features.empty, finalAmount, finalExpiry, paymentSecret))
  ...
  test("relay a trampoline htlc-add at the final node to the payment handler") { f =>
    ...
    val Success(trampolineOnion) = buildOnion(PaymentOnionCodecs.trampolineOnionPayloadLength, Seq(finalTrampolinePayload), paymentHash)
    ...
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, priv_a.privateKey, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, Seq(channelHopFromUpdate(priv_a.publicKey, b, channelUpdate_ab))), recipient)
  ...
  test("fail to relay an htlc-add with invalid blinding data (introduction node)") { f =>
    ...
    val (invoice, blindedHop, recipient) = blindedRouteFromHops(finalAmount, finalExpiry, Seq(channelHopFromUpdate(b, c, channelUpdate_bc)), routeExpiry, paymentPreimage, hex"deadbeef")
    val route = Route(finalAmount, Seq(channelHopFromUpdate(priv_a.publicKey, b, channelUpdate_ab), blindedHop))
  ...
  test("fail to relay a trampoline htlc-add when trampoline is disabled", Tag("trampoline-disabled")) { f =>
    ...
    val recipient = ClearTrampolineRecipient(invoice, finalAmount, finalAmount, finalExpiry, NodeHop(b, c, channelUpdate_bc.cltvExpiryDelta, fee_b), randomBytes32())
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, priv_a.privateKey, Upstream.Local(UUID.randomUUID()), paymentHash, Route(recipient.targetTotalAmount, Seq(channelHopFromUpdate(priv_a.publicKey, b, channelUpdate_ab))), recipient)
