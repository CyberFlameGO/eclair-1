...
import fr.acinq.eclair.wire.protocol.LightningMessageCodecs.{featuresCodec, lengthPrefixedFeaturesCodec}
...
object OfferCodecs {
  ...
  private val pathCodec: Codec[BlindedRoute] =
    (("firstNodeId" | publicKey) ::
      ("blinding" | publicKey) ::
      ("path" | listOfN(uint8, blindedNodeCodec).xmap[Seq[BlindedNode]](_.toSeq, _.toList))).as[BlindedRoute]
  ...
  private val invoiceCode: Codec[InvoiceCode] = tlvField(utf8)
  ...
  val invoiceTlvCodec: Codec[TlvStream[InvoiceTlv]] = TlvCodecs.tlvStream[InvoiceTlv](discriminated[InvoiceTlv].by(varint)
    ...
    .typecase(UInt64(176), invoiceCode)
    .typecase(UInt64(240), signature)
  ).complete
