...
class PaymentLifecycle(nodeParams: NodeParams, cfg: SendPaymentConfig, router: ActorRef, register: ActorRef) extends FSMDiagnosticActorLogging[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_REQUEST) {
    case Event(request: SendPaymentToRoute, WaitingForRequest) =>
      ...
      request.route.fold(
        hops => router ! FinalizeRoute(request.amount, hops, request.recipient.extraEdges, paymentContext = Some(cfg.paymentContext)),
        route => self ! RouteResponse(route :: Nil)
      )
      if (cfg.storeInDb) {
        paymentsDb.addOutgoingPayment(OutgoingPayment(id, cfg.parentId, cfg.externalId, paymentHash, PaymentType.Standard, request.amount, request.recipient.totalAmount, request.recipient.nodeId, TimestampMilli.now(), cfg.invoice, OutgoingPaymentStatus.Pending))
      }
    ...
    case Event(request: SendPaymentToNode, WaitingForRequest) =>
      log.debug("sending {} to {}", request.amount, request.recipient.targetNodeId)
      router ! RouteRequest(nodeParams.nodeId, request.recipient.targetNodeId, request.amount, request.maxFee, request.recipient.extraEdges, routeParams = request.routeParams, paymentContext = Some(cfg.paymentContext))
      if (cfg.storeInDb) {
        paymentsDb.addOutgoingPayment(OutgoingPayment(id, cfg.parentId, cfg.externalId, paymentHash, PaymentType.Standard, request.amount, request.recipient.totalAmount, request.recipient.nodeId, TimestampMilli.now(), cfg.invoice, OutgoingPaymentStatus.Pending))
      }
      goto(WAITING_FOR_ROUTE) using WaitingForRoute(request, Nil, Ignore.empty)
  ...
  when(WAITING_FOR_PAYMENT_COMPLETE) {
    ...
    case Event(RES_ADD_SETTLED(_, htlc, fulfill: HtlcResult.Fulfill), d: WaitingForComplete) =>
      ...
      val p = PartialPayment(id, d.request.amount, d.cmd.amount - d.request.amount, htlc.channelId, Some(d.recipient.fullRoute(d.route)))
  ...
  private def retry(failure: PaymentFailure, data: WaitingForComplete): FSM.State[PaymentLifecycle.State, PaymentLifecycle.Data] = {
    data.request match {
      case request: SendPaymentToNode =>
        ...
        router ! RouteRequest(nodeParams.nodeId, data.recipient.targetNodeId, data.request.amount, request.maxFee, data.recipient.extraEdges, ignore1, request.routeParams, paymentContext = Some(cfg.paymentContext))
  ...
  private def handleLocalFail(d: WaitingForComplete, t: Throwable, isFatal: Boolean) = {
    t match {
      ...
      case _ => Metrics.PaymentError.withTag(Tags.Failure, Tags.FailureType(LocalFailure(d.request.amount, d.recipient.fullRoute(d.route), t))).increment()
    }
    ...
    val localFailure = LocalFailure(d.request.amount, d.recipient.fullRoute(d.route), t)
    if (doRetry) {
  ...
  private def handleRemoteFail(d: WaitingForComplete, fail: UpdateFailHtlc) = {
    ...
    ((Sphinx.FailurePacket.decrypt(fail.reason, sharedSecrets) match {
      ...
      case res@Success(Sphinx.DecryptedFailurePacket(nodeId, failureMessage)) =>
        ...
        failureMessage match {
          case TemporaryChannelFailure(update) =>
            route.hops.collectFirst { case hop: ChannelHop if hop.nodeId == nodeId => hop } match {
      ...
      case Success(e@Sphinx.DecryptedFailurePacket(nodeId, failureMessage)) if nodeId == recipient.targetNodeId =>
        // if destination node returns an error, we fail the payment immediately
        log.warning(s"received an error message from target nodeId=$nodeId, failing the payment (failure=$failureMessage)")
        myStop(request, Left(PaymentFailed(id, paymentHash, failures :+ RemoteFailure(request.amount, recipient.fullRoute(route), e))))
      case res if failures.size + 1 >= request.maxAttempts =>
        ...
        val failure = res match {
          case Success(e@Sphinx.DecryptedFailurePacket(nodeId, failureMessage)) =>
            ...
            RemoteFailure(request.amount, recipient.fullRoute(route), e)
          case Failure(t) =>
            log.warning(s"cannot parse returned error ${fail.reason.toHex} with sharedSecrets=$sharedSecrets: ${t.getMessage}")
            UnreadableRemoteFailure(request.amount, recipient.fullRoute(route))
        }
      ...
      case Failure(t) =>
        ...
        val failure = UnreadableRemoteFailure(request.amount, recipient.fullRoute(route))
        retry(failure, d)
      ...
      case Some(hop) => hop.params match {
        ...
        case _: HopRelayParams.FromHint =>
          ...
          if (failure.update.channelFlags.isEnabled) {
            ...
            data.recipient.extraEdges.collect {
            ...
            data.recipient.extraEdges.filter(edge => edge.sourceNodeId != nodeId || edge.targetNodeId != hop.nextNodeId)
          }
      }
      case None =>
      ...
      val fees = result match {
        ...
        case Left(paymentFailed) =>
          val (fees, pathFindingExperiment) = request match {
            case s: SendPaymentToNode => (request.recipient.targetMaxFee(s.routeParams), s.routeParams.experimentName)
...
object PaymentLifecycle {
  ...
  sealed trait SendPayment {
    ...
    /** Amount to send to the target node. */
  ...
  case class SendPaymentToRoute(replyTo: ActorRef,
                                ...
                                amount: MilliSatoshi,
                                recipient: Recipient) extends SendPayment {
  ...
  case class SendPaymentToNode(replyTo: ActorRef,
    ...
    require(amount > 0.msat, "amount must be > 0")
    val maxFee = recipient.targetMaxFee(routeParams)
  }
