...
class PaymentInitiator(nodeParams: NodeParams, outgoingPaymentFactory: PaymentInitiator.MultiPartPaymentFactory) extends Actor with ActorLogging {
  ...
  def main(pending: Map[UUID, PendingPayment]): Receive = {
    case r: SendPaymentToNode =>
      ...
      if (!nodeParams.features.invoiceFeatures().areSupported(recipient.features)) {
        ...
        fsm ! PaymentLifecycle.SendPaymentToNode(self, r.recipientAmount, recipient, r.maxAttempts, r.routeParams)
        context become main(pending + (paymentId -> PendingPaymentToNode(sender(), r)))
      }
    ...
    case r: SendSpontaneousPayment =>
      val paymentId = UUID.randomUUID()
      sender() ! paymentId
      val paymentCfg = SendPaymentConfig(paymentId, paymentId, r.externalId, r.paymentHash, r.recipientNodeId, Upstream.Local(paymentId), None, storeInDb = true, publishEvent = true, recordPathFindingMetrics = r.recordPathFindingMetrics)
    ...
    case r: SendPaymentToRoute =>
      ...
      r.trampoline_opt match {
        case Some(trampolineAttempt) =>
          ...
          buildTrampolinePayment(r, trampolineHop) match {
            case Success(recipient) =>
              ...
              // The requested amount already includes the trampoline fee to avoid counting it multiple times.
              val recipientAmount = r.amount - trampolineAttempt.fees
              payFsm ! PaymentLifecycle.SendPaymentToRoute(self, Left(r.route), recipientAmount, recipient)
        ...
        case None =>
          ...
          val payFsm = outgoingPaymentFactory.spawnOutgoingPayment(context, paymentCfg)
          payFsm ! PaymentLifecycle.SendPaymentToRoute(self, Left(r.route), r.amount, recipient)
          context become main(pending + (paymentId -> PendingPaymentToRoute(sender(), r)))
        case _ =>
  ...
  private def buildTrampolinePayment(r: SendRequestedPayment, trampolineHop: NodeHop): Try[ClearTrampolineRecipient] = {
    ...
    val finalExpiry = r.finalExpiry(nodeParams)
    r.invoice match {
      case invoice: Bolt11Invoice => Success(ClearTrampolineRecipient(invoice, r.recipientAmount, r.recipientAmount, finalExpiry, trampolineHop, trampolineSecret))
...
object PaymentInitiator {
   ...
   * SendPaymentToRoute(200 msat, 600 msat, invoice, Seq(alice, bob, dave), None, Some(parentId), None)
   * SendPaymentToRoute(400 msat, 600 msat, invoice, Seq(alice, carol, dave), None, Some(parentId), None)
   *
   * Example 2: Trampoline with MPP for a 600 msat invoice and 100 msat trampoline fees:
   * SendPaymentToRoute(250 msat, 600 msat, invoice, Seq(alice, bob, ted), None, Some(parentId), Some(TrampolineAttempt(secret, 100 msat, CltvExpiryDelta(144))))
   * SendPaymentToRoute(450 msat, 600 msat, invoice, Seq(alice, carol, ted), None, Some(parentId), Some(TrampolineAttempt(secret, 100 msat, CltvExpiryDelta(144))))
   *
